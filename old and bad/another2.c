...
BloomFilter* create_bloom_filter(size_t bit_size, int hash_count) {
    BloomFilter *filter = (BloomFilter*)malloc(sizeof(BloomFilter));
    if (!filter) {
        perror("Error allocating memory for BloomFilter");
        exit(EXIT_FAILURE);
    }
    filter->size = bit_size;
    size_t byte_size = (bit_size + 7) / 8; // Convert bit size to byte size, ensuring we allocate enough bytes
    filter->hash_count = hash_count;
    filter->bit_array = (unsigned char*)calloc(byte_size, sizeof(unsigned char));
    if (!filter->bit_array) {
...
int main() {
    ...
    char **rockyou_words = malloc(sizeof(char*) * 17479516);
    if (!rockyou_words) {
