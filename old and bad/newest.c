...
void test_rockyou(const char *file_path, BloomFilter *filter, Results *results) {
    FILE *file = fopen(file_path, "r");
    if (!file) {
        perror("Error opening file");
        exit(EXIT_FAILURE);
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        printf("Testing: %s - Result: %d\n", line, result); // Debug statement
        if (result) {
            if (strcmp(line, "12345") == 0) { // Debug specific check
                printf("Specific check: Found '12345'\n");
            }
            results->true_positive++; // Updated to test for true positives
            printf("maybe\n");
        } else {
...
int main() {
    ...
    BloomFilter *bloom_filter = create_bloom_filter(139836133, 1); // Updated size and hash_count
    load_bloom_filter(dictionary_file, bloom_filter);

    Results results = {0, 0, 0, 0};
    test_rockyou(rockyou_file, bloom_filter, &results);
